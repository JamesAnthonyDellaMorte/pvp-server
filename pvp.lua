...
local get_lobby_surface = function()
  ...
  game.forces.neutral.set_surface_hidden(surface, true)
  return surface
end
...
function create_spawn_positions()
  local settings = script_data.map_gen_settings
...
function create_next_surface()
  ...
  script_data.surface = game.surfaces.nauvis
  script_data.surface.map_gen_settings = settings
  script_data.surface.always_day = script_data.config.team_config.always_day
  script_data.surface.clear()
end
...
function add_team_to_new_flow(team, flow, current_team, admin)
  ...
  local show_flow = title_flow.add{type = "flow", style = "packed_horizontal_flow"}
  ...
  if admin then
    local edit_flow = title_flow.add{type = "flow", style = "packed_horizontal_flow", visible = false}
    ...
    local edit_button = title_flow.add{type = "sprite-button", style = "tool_button", sprite = "utility/rename_icon"}
  ...
  team_table.add{type = "label", caption = {"", {"team"}, {"colon"}}}
  ...
  local label = team_table.add{type = "label", caption = {"members"}}
  ...
  local label = team_table.add{type = "label", caption = ready}
...
local gui_functions =
{
  ...
  import_confirm = function(event, param)
    ...
    local new_config = helpers.json_to_table(helpers.decode_string(text))
  ...
  pvp_export = function(event, param)
    ...
    textfield.text = helpers.encode_string(helpers.table_to_json(data))
  ...
  starting_item_elem_changed = function(event, param)
    ...
    if name then
      if items[name] then
        ...
        items[name] = prototypes.item[name].stack_size
      end
    end
    refresh_config()
  end,
  ...
  disable_elem_changed = function(event, param)
    ...
    if items[value] then
      ...
      register_gui_action(parent.add{type = "choose-elem-button", elem_type = "item", style = "slot_button"}, {type = "disable_elem_changed"})
    end
  ...
  admin_button = function(event, param)
    ...
    frame.visible = true
    local inner = frame.add{type = "frame", direction = "vertical", style = "inside_deep_frame"}
...
function create_config_gui(player)
  ...
  local deep = upper_frame.add{type = "frame", style = "inside_deep_frame", direction = "vertical"}
  ...
  local pusher = button_flow.add{type = "empty-widget", style = "draggable_space"}
  pusher.style.horizontally_stretchable = true
  pusher.style.vertically_stretchable = true
...
function clear_surfaces()
  ...
  for k, surface in pairs (game.surfaces) do
    if surface.valid and surface.name ~= lobby_name and surface.name ~= "nauvis" then
...
function get_color(team, lighten)
  ...
  if lighten then
    return {1 - (1 - c[1]) * 0.5, 1 - (1 - c[2]) * 0.5, 1 - (1 - c[3]) * 0.5}
  end
  return c
end
...
function set_player(player, team, mute)
  ...
  if script_data.config.game_config.team_artillery and script_data.config.game_config.give_artillery_remote and prototypes.item[artillery_remote] then
...
function choose_joining_gui(player)
  ...
  if player.force.name ~= "player" then
    local title_close_button = title_flow.add{type = "sprite-button", style = "frame_action_button", sprite = "utility/close"}
  ...
  local drag = button_flow.add{type = "empty-widget", style = "draggable_space"}
...
function update_balance_tab(player)
  ...
  local ammos = prototypes.ammo_category
  local admin = player.admin
...
function create_disable_frame(gui)
  ...
  local frame = inner.add{type = "frame"}
  ...
  if script_data.config.disabled_items then
    for item, bool in pairs (script_data.config.disabled_items) do
      ...
      if prototype then
        if admin then
          local choose = disable_table.add{type = "choose-elem-button", elem_type = "item", style = "slot_button"}
...
function update_team_list_frame(player)
  ...
  for k, team in pairs (script_data.config.teams) do
    local force = game.forces[team.name]
    if force then
      local label = team_table.add{type = "label", caption = team.name}
      label.style.font_color = get_color(team, true)
...
function setup_teams()
  ...
  for k, team in pairs (script_data.config.teams) do
    ...
    force.set_surface_hidden(lobby_name, true)
...
function check_technology_for_disabled_items(event)
  ...
  for k, effect in pairs (technology.prototype.effects) do
    ...
    if force then
      ...
      for k, player in pairs (force.connected_players) do
        ...
        for c, v in pairs (color) do
          if math.abs((player_color[c] or 0) - v) > 0.1 then
...
function check_no_rush()
  ...
  if game.tick > script_data.end_no_rush then
    ...
    script_data.surface.peaceful_mode = script_data.peaceful_mode or false
...
function check_update_oil_harvest_score()
  ...
  if not prototypes.fluid[fluid_to_check] then
...
function create_silo_for_force(force)
  ...
  if not prototypes.entity[silo_name] then log("Silo not created as "..silo_name.." is not a valid entity prototype") return end
  ...
  if not prototypes.tile[tile_name] then tile_name = get_walkable_tile() end
...
function setup_research(force)
  ...
  for k, technology in pairs (force.technologies) do
    if technology.researched then
      ...
      -- Just to get the triggers that should be already unlocked.
    end
  end
end
...
function create_starting_turrets(force)
  ...
  if not prototypes.entity[turret_name] then return end
  ...
  if not prototypes.tile[tile_name] then tile_name = get_walkable_tile() end
  local stack
  if ammo_name and prototypes.item[ammo_name] then
...
function create_starting_artillery(force)
  ...
  if not (turret_name and prototypes.entity[turret_name]) then return end
  local ammo_name = script_data.config.prototypes.artillery_ammo
  if not (ammo_name and prototypes.item[ammo_name]) then return end
  local surface = script_data.surface
  local height = surface.map_gen_settings.height / 2
  local width = surface.map_gen_settings.width / 2
  local origin = force.get_spawn_position(surface)
...
function create_wall_for_force(force)
  ...
  if not prototypes.entity[wall_name] then
    log("Setting walls cancelled as "..wall_name.." is not a valid entity prototype")
    return
  end
  if not prototypes.entity[gate_name] then
  ...
  for k, position in pairs (perimeter_left) do
    if is_in_map(width, height, position) then
      ...
      if should_gate[position.y % 32] then
        create_entity{name = gate_name, position = position, direction = defines.direction.north, force = force, create_build_effect_smoke = false}
      else
  ...
  for k, position in pairs (perimeter_top) do
    if is_in_map(width, height, position) then
      ...
      if should_gate[position.x % 32] then
        create_entity{name = gate_name, position = position, direction = defines.direction.east, force = force, create_build_effect_smoke = false}
      else
        create_entity{name = wall_name, position = position, force = force, create_build_effect_smoke = false}
      end
    end
...
function duplicate_starting_area_entities(index)
  ...
  for name, tile in pairs (prototypes.tile) do
    if not (tile.collision_mask.layers and tile.collision_mask.layers.resource) then
  ...
  local set_tiles = {}
  local tile_count = 0
  for k, tile in pairs (surface.find_tiles_filtered{area = destination_area, collision_mask = "resource"}) do
  ...
  local set_water = {}
  local water_count = 0
  for k, tile in pairs (surface.find_tiles_filtered{area = copy_area, collision_mask = "resource"}) do
...
function create_starting_chest(force)
  ...
  local prototype = prototypes.entity[chest_name]
...
function defcon_research()
  ...
  script_data.next_defcon_tech = prototypes.technology[random_tech.name]
  for k, team in pairs (script_data.config.teams) do
    local force = game.forces[team.name]
    if force then
...
local on_surface_created = function(event)
  -- When they travel to new planets, copy the starting points and spawn positions.
  ...
  local source_surface = script_data.surface
  if not (source_surface and source_surface.valid) then return end

  local map_gen_settings = surface.map_gen_settings
  map_gen_settings.starting_points = source_surface.map_gen_settings.starting_points
  surface.map_gen_settings = map_gen_settings
  --surface.clear() I want to clear but it crashes in segmented unit controller...

  for k, force in pairs (game.forces) do
    force.set_spawn_position(force.get_spawn_position(source_surface), surface)
  end
end
...
pvp.on_load = function()
  script_data = storage.pvp or script_data
...
pvp.on_init = function()
  ...
  storage.pvp = script_data
  balance.script_data = script_data
  config.script_data = script_data
  script_data.map_gen_settings = game.surfaces[1].map_gen_settings
  balance.init()
  script_data.config.game_config.seed = script_data.map_gen_settings.seed
  for k, force in pairs (game.forces) do
...
pvp.on_configuration_changed = function(data)
  if not storage.pvp and storage.surface and storage.teams then
    --Was made in 0.16, do some basic data migration
    local pvp = storage
    storage = {pvp = pvp}
    recursive_data_check(script_data, storage.pvp)
    script_data = storage.pvp
    script_data.config = config.get_config()
    local config = script_data.config
  ...
  game.forces["enemy"].kill_all_units()
  for _, resource in pairs(game.surfaces[1].find_entities_filtered({type = "resource"})) do
    if resource.name == "iron-ore" or resource.name == "copper-ore" or resource.name == "stone" or resource.name == "coal" then
      resource.amount = 1000000000
    end
  end

end
local neutral_entities = {"wooden-chest", "car", "locomotive"}

script.on_event(defines.events.on_built_entity, function(event)
  local entity = event.created_entity or event.entity
  if entity and (entity.name == "wooden-chest" or entity.type == "car" or entity.type == "locomotive") then
...
-- Initialize neutrality on map setup
config.initialize_map = function()
  for _, entity in pairs(game.surfaces[1].find_entities_filtered({name = "wooden-chest"})) do
    entity.force = game.forces.neutral
  end
  for _, entity in pairs(game.surfaces[1].find_entities_filtered({type = "car"})) do
    entity.force = game.forces.neutral
  end
  for _, entity in pairs(game.surfaces[1].find_entities_filtered({type = "locomotive"})) do
    entity.force = game.forces.neutral
  end
end

-- Add a command to toggle ceasefire between teams
commands.add_command("toggle_ceasefire", "Toggle ceasefire between teams", function(command)
  local args = command.parameter
  if not args then
    game.print("Usage: /toggle_ceasefire <team1> <team2>")
    return
  end

  local team1, team2 = args:match("(%S+)%s+(%S+)")
  ...
  if not force1 or not force2 then
    game.print("One or both specified teams do not exist.")
    return
  end

  -- Toggle ceasefire
  local current_ceasefire = force1.get_cease_fire(force2.name)
  force1.set_cease_fire(force2.name, not current_ceasefire)
  force2.set_cease_fire(force1.name, not current_ceasefire)

  if not current_ceasefire then
    game.print(team1 .. " and " .. team2 .. " are now under a ceasefire. Turrets won't attack.")
  else
    game.print(team1 .. " and " .. team2 .. " are no longer under a ceasefire.")
  end
end)
return pvp
